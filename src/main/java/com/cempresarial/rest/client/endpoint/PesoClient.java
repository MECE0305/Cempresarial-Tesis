 ...
 * @author DIGETBI 05
 */
public class PesoClient extends AbstractClient {

	private static final Logger log = Logger.getLogger(PesoClient.class.getName());

	public PesoClient(String url, String contextPath) {
		super(url, contextPath);
	}

	public List<Peso> listar() throws ServiceException {
		...
		if (200 == status) {
			...
			list = new Gson().fromJson(resultado, new TypeToken<ArrayList<Peso>>() {
			}.getType());
	...
	public Peso actualizar(Long id, Peso entidad) {
		...
		Peso obj = new Gson().fromJson(resultado, Peso.class);
	...
	public Peso crear(Peso entidad) {
